<?php
require("cont.php");
?>
<html>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="Stylesheet" href="contact.css">
  <script src="contact.js"></script>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
  <head></head>
  <script>
</script>
  <title>Home contact</title>
  <body class="main" id="demo">
  
    <div class="dec">
        <br>
        <img src="h.jfif" style="width:50px;height:50px;float:left;margin-left: 9px;border-radius: 40px;margin-top: -13px;">
        <h>WELCOME TO HOME BOOKING</h>
      </div>
      <div class="con" style="width:400;height: 500px;">
      <form action=" " method="post" onsubmit="return valid()">
        <p><img src="u.jfif" class='responsive'style="width:100px;height:100px;"></p>
        <br>
        <br>
        <label style="margin-left: -140px;">Name</label>    <label style="margin-left: 140px;">Email</label><br>
        <input type="text" name="nm" id="nm"> <input type="email" name="em" id="em">
        <br>
        <br>
        <label style="margin-left: -130px;">Contact</label>    <label style="margin-left: 130px;">Adress</label><br>
        <input type="number" name="mb" id="mb"> <input type="text" name="ad" id="ad">
        <br>
        <br>
        <br>
        <input type="submit" value="Click" >
       </form>
      </div>
      <br>
      <br>
      <br>
      <div class="foot">
        <br>
        Author: More Mahesh <br>
        <a href="moremk2019@gmail.com">moremk2019@gmail.com</a>
        <br>
          <a href="https://www.instagram.com/soft_code_mahesh"><i class="fa fa-instagram" aria-hidden="true" style="background-color:rgb(228, 64, 95);border-radius: 5px;font-size: 30px;color:white;"></i></a>
       <a href="https://wa.me/+919322974643"><i class="fa fa-whatsapp" aria-hidden="true" style="background-color: rgb(37, 211, 102);border-radius:5px; font-size: 30px;color:white;"></i></a>
      </div>
  </body>
</html>
